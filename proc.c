 ...
 */
  ...
  register struct proc *rp2;
  struct proc **rdy_head;
  int q;	
  int fss_priority;			/* iterate over queues */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	...
	if(q == USER_Q){
		fss_priority = rp->p_fss_priority;
		rp2 = rp;
		while(rp != NULL){
			if(fss_priority < rp->p_fss_priority){
				fss_priority = rp->p_fss_priority;
				rp2 = rp;
			}
			rp=rp->p_nextready;
		}
		assert(proc_is_runnable(rp2));
		if (priv(rp2)->s_flags & BILLABLE)	 	
			get_cpulocal_var(bill_ptr) = rp2;
		return rp2;		
